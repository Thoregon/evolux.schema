...
export const CHILD  = type => new Type({ id: 'CHILD', name: 'child', description: 'embedded child entity. does not provide own commands', type: type });
...
export const COUNTER    = new Type({ id: 'COUNTER', name: 'Integer', format: '#,##0' })

/*
 * Container types
 */
export const LIST =     content         => new ContainerType({ id: 'LIST', name: 'List', content });
export const MAP =      (content, key)  => new ContainerType({ id: 'MAP', name: 'Map', content, key: key });  // use the internal id if not specified
